...
const heroSlides = [
  {
    title: 'Discover Quality Plastic Products for Every Need',
    subtitle: 'Shop the best deals on household, industrial, and custom plastic items. Fast delivery, top brands, and unbeatable prices!',
    image: plasticbg1,
    cta: 'Shop Now',
    link: '/app/shop',
  },
  ...
  {
    title: 'Eco-Friendly & Durable Plastics',
    subtitle: 'Explore our range of sustainable, long-lasting plastic products for every application.',
    image: plasticbg2,
    cta: 'Browse Products',
    link: '/app/shop',
  },
  ...
  return (
    <section className="relative bg-gradient-to-br from-blue-50 to-black py-16 md:py-24 lg:py-32 flex flex-col items-center justify-center text-center overflow-hidden min-h-[420px]">
      ...
      <div className="max-w-3xl mx-auto z-10 relative">
        ...
        <Link
          to={slide.link}
          className="inline-block bg-orange-500 hover:bg-orange-600 text-white font-semibold text-lg px-8 py-3 rounded-full shadow transition duration-200"
        >
          {slide.cta}
        </Link>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2 z-10">
        {heroSlides.map((_, idx) => (
          <button
            key={idx}
            className={`w-3 h-3 rounded-full border-2 border-blue-400 bg-white transition-all duration-200 ${current === idx ? 'bg-blue-600 border-blue-600' : ''}`}
      ...
      <div className="absolute right-0 bottom-0 w-56 h-56 bg-blue-200 rounded-full opacity-20 -z-1 animate-pulse" style={{ filter: 'blur(18px)' }} />
    </section>
  );
}
