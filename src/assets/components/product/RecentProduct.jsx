...
const RecentProduct = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(`${API_BASE_URL}/products/?limit=10&sortBy=createdAtDesc`);
  ...
  return (
    <div className="font-sans antialiased bg-white p-6 md:p-10 lg:p-12">
      ...
      {isError && <div className="text-center text-red-600 py-8">{error?.message || 'Failed to load products.'}</div>}
