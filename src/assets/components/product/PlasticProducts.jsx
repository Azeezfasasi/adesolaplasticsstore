...
const PlasticProducts = () => {
  ...
  const { data: chairCategory, isLoading: loadingCategory, isError: errorCategory, error: categoryError } = useQuery({
    queryKey: ['category', 'chairs'],
    queryFn: async () => {
      const res = await axios.get(`${API_BASE_URL}/categories/slug/chairs`);
      return res.data;
    },
  });
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['products-chairs', chairCategory?._id],
    enabled: !!chairCategory?._id,
    queryFn: async () => {
      const res = await axios.get(`${API_BASE_URL}/products?category=${chairCategory._id}&limit=5`);
