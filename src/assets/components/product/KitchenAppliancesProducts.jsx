...
const KitchenAppliancesProducts = () => {
  // Fetch the 'chairs' category first
  const { data: kitchenAppliancesCategory, isLoading: loadingCategory, isError: errorCategory, error: categoryError } = useQuery({
    queryKey: ['category', 'kitchen-appliances'],
    queryFn: async () => {
      const res = await axios.get(`${API_BASE_URL}/categories/slug/kitchen-appliances`);
      return res.data;
    },
  });
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['products-kitchen-appliances', kitchenAppliancesCategory?._id],
    enabled: !!kitchenAppliancesCategory?._id,
    queryFn: async () => {
      const res = await axios.get(`${API_BASE_URL}/products?category=${kitchenAppliancesCategory._id}&limit=5`);
...
export default KitchenAppliancesProducts;
