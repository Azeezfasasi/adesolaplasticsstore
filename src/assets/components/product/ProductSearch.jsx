    ...
    <div className="relative w-full max-w-4xl mx-auto my-2 px-2 md:px-0">
      <form onSubmit={e => e.preventDefault()} className="flex items-center gap-2">
        <input
          ...
          className="flex-1 px-4 py-2 border border-gray-300 rounded-l-md focus:outline-none focus:ring-2 focus:ring-orange-500 text-base"
        />
        <button
          type="submit"
          className="px-5 py-2 bg-orange-500 text-white font-semibold rounded-r-md hover:bg-orange-600 transition"
          disabled
        >
          Search
        </button>
      ...
      {(loading || error || (results.length > 0 && query.trim())) && (
        <div className="absolute left-0 right-0 mt-1 bg-white rounded-xl shadow-lg z-50 overflow-hidden max-h-80 overflow-y-auto">
          {loading && <div className="text-center py-4 text-orange-500">Searching...</div>}
              ...
              {results.map(product => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-orange-500 font-bold text-base">₦{product.salePrice || product.price}</div>
                  </div>
                </Link>
              ))}
            </div>
